...
var ros_rslidar = RSlidars.new()
var sensor_array = []

const RaySensorResource = preload("res://scenes/main.tscn")
...
var HORIZONTAL_OPERTURE_ANGLE = deg_to_rad(H_ANGLE_OP)
var delta_h = 2 * HORIZONTAL_OPERTURE_ANGLE / (MAX_COL - 1)

var VERTICAL_OPERTURE_ANGLE = deg_to_rad(V_ANGLE_OP)
...
func create_ray_matrix():
	...
	for i in range(number_of_sensors):
		var RaySensorInstance = RaySensorResource.instantiate()  # Use instantiate() instead of instance()
		RaySensorInstance.name = "RaySensor" + str(i)
		
		# Removed the unused 'vector' variable
		# var vector = Vector3(col_num * sensor_interspace, row_num * sensor_interspace, 0.0)

		# Apply rotations using Quat for local rotations
		var rotation = Quaternion()
		rotation = rotation.rotated(Vector3(0, 1, 0), horizontal_angle)  # Rotate around Y-axis (horizontal)
		rotation = rotation.rotated(Vector3(1, 0, 0), vertical_angle)  # Rotate around X-axis (vertical)
		RaySensorInstance.transform.basis = rotation

		# Set the position of the RaySensorInstance
		RaySensorInstance.transform.origin = Vector3(col_num * sensor_interspace, row_num * sensor_interspace, 0.0)
...
func update_sensordata():
	...
	for sensor in ray_sensor_array:
		...
		if sensor.is_colliding():  # Use is_colliding() instead of collides()
	...
	ros_rslidar.publish_raycast_msg(sensor_array)
